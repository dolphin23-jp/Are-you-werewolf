...
PARALLEL_GAMES="${WEREWOLF_AUDIT_PARALLEL_GAMES:-32}"
INFERENCE_BATCH_SIZE="${WEREWOLF_AUDIT_INFERENCE_BATCH_SIZE:-128}"
WORKERS="${WEREWOLF_AUDIT_WORKERS:-4}"
INFERENCE_COALESCE_MS="${WEREWOLF_AUDIT_INFERENCE_COALESCE_MS:-4}"
...
worker() {
  ...
  env PYTHONPATH=. python scripts/audit_population_retention_torch.py \
    ...
    --workers "$WORKERS" \
    --inference-coalesce-ms "$INFERENCE_COALESCE_MS" \
    --device cuda
}

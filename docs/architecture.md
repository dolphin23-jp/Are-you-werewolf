...
- **フェーズ別並行性ポリシー**: 議論=有限キュー付き逐次、投票=並列、夜=依存関係あり。
  議論では全AIが1回ずつ発言した後、すでに発言済みのAIへ明示的な質問があった場合だけ
  返信キューへ積む。同一AIの返信は1回、1ターンの追加返信は全体4回までなので、AI同士の
  応酬が続いても必ず停止して人間へ制御を返す。夜は占い/護衛/内輪チャットを並列実行し、
  人狼の襲撃決定はそれらを踏まえて後段で行う。

...
def evaluate_torch_policy_profiles_multiprocess(
    ...
    *,
    ...
    """Evaluate games in CPU workers while keeping all model/CUDA work in parent.

    The total number of simultaneously active games remains bounded by
    ``max_parallel_games``. That chunk is partitioned over spawn-based CPU worker
    processes. Workers own environments, observations, encoding, legality and
    sampling. They send only encoded observations plus immutable policy ids to
    the parent, which is the sole checkpoint/model/accelerator owner.
    """
...
def _worker_main(
    ...
    def remote_infer(
        policy_id: str | object,
        observations: tuple[EncodedPolicyObservation, ...],
    ) -> tuple[PolicyLogits, ...]:
        nonlocal request_id
        if not isinstance(policy_id, str):
            raise TypeError("multiprocess worker requires string policy ids")
        ...
        if reply.request_id != current_request:
    ...
    while True:
        ...
        try:
            ...
            default_policy_id = job.games[0].profile.village
            collector = TorchVectorizedEpisodeCollector(
                ...
                inference_fn=remote_infer,
            )
            team_models = tuple(
                {
                    Team.VILLAGE: game.profile.village,
                    Team.WEREWOLF: game.profile.werewolf,
                    Team.FOX: game.profile.fox,
                }
    ...
    *,
    ...
    for message in messages:
        ...
        if message.policy_id not in model_cache:
            raise RuntimeError(
                f"worker requested unloaded policy {message.policy_id}"
            )
        requests.append(message)

    inference_stats.record_pending(
        sum(len(request.observations) for request in requests)
    )

...
def test_day_one_morning_names_the_first_victim():
    """The first victim was filtered out of the day-1 morning list."""
    controller = make_controller(seed=4)
    controller.start_game()
    controller.resolve_night()
    controller.start_discussion()
    state = controller.state
    first = state.first_victim_id
    assert first is not None

    rendered = _builder(state)._layer_c_state(state, "p1", [])

    assert "【今朝の公開死体" in rendered
    assert f"{state.players[first].name}({first})(初日犠牲者)" in rendered
